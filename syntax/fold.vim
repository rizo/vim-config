...
" Statement
syn keyword foldStatement assert ignore
...
" Keyword
syn keyword foldKeyword do end var def val and value with catch where let in quote lambda object rec function func fun fn interface protocol instance module mod signature sig ext extension macro

" Exception
syn keyword foldException exception raise try
...
" syn match foldFunctionDefName   "\v(fun!=\s+)@<=\w+"
" syn match foldFunctionDefName   "\v(let!=\s+)@<=\w+"
" syn match foldFunctionDefName   "\v(val!=\s+)@<=\w+"
